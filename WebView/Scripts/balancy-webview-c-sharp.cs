...
namespace Balancy.WebView
{
    ...
    public class BalancyWebView : MonoBehaviour
    {
        ...
        private float _showDelay = 0f; // Immediate by default
        private float _animationDuration = 0f; // No fade by default
        ...
        private class PersistentMessage
        {
            ...
            public string htmlBase64, ownerJsonBase64, additionalInfoBase64, scriptsBase64, baseUrl, scriptsVersion;
        }
        ...
        {
            if (_isWebViewOpen || (_persistent?.Enabled ?? false))
            {
                CloseWebView();
            }
        ...
        public bool OpenWebView(string url, string ownerJson, string additionalInfo, int width, int height)
        {
            ...
            // Native controllers may be created by the open call; settings sent
        ...
        public void PrepareWebView(Action onReady = null, Action<string> onFailed = null)
        {
#if UNITY_EDITOR_OSX || (!UNITY_EDITOR && (UNITY_IOS || UNITY_ANDROID || UNITY_WEBGL || UNITY_STANDALONE_OSX))
            if (!Persistent.Enabled && (_isWebViewOpen || _isWebViewEmbedded))
            {
            ...
            Persistent.Prepare(() => {
                ...
                return started;
#endif
            }, onReady, error => { Debug.LogError("[BalancyWebView] " + error); onFailed?.Invoke(error); });
#else
            const string error = "Persistent WebView is not supported on this platform";
        ...
        public void InvalidateCache(bool localizationOnly = false)
        {
            if (Persistent.Enabled && !Persistent.Preparing)
                SendMessageToWebView("{\"type\":\"invalidateCache\",\"scope\":\"" + (localizationOnly ? "localization" : "all") + "\"}");
        }
        ...
        public bool SendMessageToWebView(string message)
        {
            ...
            bool canUsePersistentWebView = Persistent.Enabled;
        ...
        /// </summary>
        /// <param name="delaySeconds">Delay in seconds (default: 0)</param>
        public void SetShowDelay(float delaySeconds)
        {
            ...
            if (_isWebViewOpen || _isWebViewEmbedded || (_persistent?.Enabled ?? false))
            {
        ...
        /// </summary>
        /// <param name="durationSeconds">Animation duration in seconds (default: 0)</param>
    ...
    return false;
        ...
        {
            if (_instance != null) _instance.OnMessageReceivedPrivate(message);
        }
        ...
        private void OnMessageReceivedPrivate(string message)
        {
            ...
            try
            {
                ...
                {
                    _acknowledgedScriptsVersion = parsed.scriptsVersion;
