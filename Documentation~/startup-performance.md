...
| Metric | SDK 1.8.2 baseline | Latest before fix | Iteration 1 | Iteration 2 |
| --- | ---: | ---: | ---: | ---: |
| Android StreamingAssets preload | 602 ms median | 726 ms | 763 ms | **715 ms median** |
| Session start to first `OnDataUpdated` | 0.955 s median | 1.410 s | 2.712 s | **2.164 s median** |
| `InitCloudVersion` to cloud `OnDataUpdated` | 3.627 s median | 12.064 s | 1.824 s | **0.861 s median** |
| Cloud `OnDataUpdated` handler | not instrumented | 97.1 ms dispatch batch | 39.0 ms | **11.5 ms median** |
| Largest logged native main-thread dispatch after first callback | not instrumented | 97.1 ms | 41.8 ms | **28.3 ms** |
| Native JavaScript injection | about 1.98M script chars, compiled twice | 2,501,715 chars plus 471,437-char bridge | 2,510,326 chars plus 471,437-char bridge | **1,139-char shell + 1,660-char bootstrap** |
| Combined script CDN request when matching file is packaged | not supported | yes | yes | **no** |
| Asset preload relative to cloud callback | blocks update path | blocks update path | asynchronous, 6.783 s | **asynchronous, 5.450 s median** |
| Crash or ANR in measured run | customer reported | none in reference run | none | **none** |
